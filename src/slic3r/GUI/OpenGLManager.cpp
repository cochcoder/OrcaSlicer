...
bool OpenGLManager::init_gl(bool popup_error)
{
    if (!m_gl_initialized) {
        ...
        if (version == 0) {
            BOOST_LOG_TRIVIAL(error) << "Unable to init GLAD library";
            return false;
        }
        BOOST_LOG_TRIVIAL(info) << "GLAD initialized successfully. OpenGL version: " << GLVersion.major << "." << GLVersion.minor << std::endl;
